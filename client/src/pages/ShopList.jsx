    ...
    return (
        <div className="container-sm table-responsive-sm shoplist-content-container">
                ...
                <tbody>
                    {shoplistData && shoplistData.products ? (
                        shoplistData.products.map((product, index) => (
                            <tr key={index}>
                                <td>{product}</td>
                                <td><input type="number" value={shoplistData.quantity && shoplistData.quantity[index]} onChange={(event) => handleQuantityChanges(event, index)} /></td>
                                <td><input type="number" value={shoplistData.price && shoplistData.price[index]} onChange={(event) => handlePriceChanges(event, index)} /></td>
                                <td>{isNaN(productTotal[index]) ? "Loading" : "$" + productTotal[index]}</td>
                                <td><button className="remove-product-btn" onClick={() => removeProduct(index)}><FaRegTrashAlt /></button></td>
                            </tr>
                        ))
                    ) : (
                        <tr>
            ...
            <div className="bottom-content">
                ...
                <button type="submit" onClick={updateShoplist}>Save changes</button>
            </div>
        </div>
    );
}
